...
spec:
  ...
  # (запрещено CEL): ёмкость — hostPath, фактор выводится из числа мастеров.
  storage:
...
Последствия смены числа мастеров у System (фактор не меняется):

- **3 → 2 → 3** (обслуживание/транзиент): rf=3, при 2 живых узлах кворум (2)
  соблюдён — кластер обслуживает чтение и запись; при 1 узле переходит в
  read-only; при возврате мастеров снова healthy. Потери данных нет.
- **1 → 3** (постоянный рост): rf остаётся закреплённым (растёт ёмкость, но не
