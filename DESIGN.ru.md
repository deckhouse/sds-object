...
- **Рост 1 → 2 → 3**: добавление мастера **не расползает** реплики само — они
  прибиты `nodeAffinity` к первому узлу. HA появляется через управляемый rebalance.
...
Мешинг реконсилит layout (`layoutRoleChanges`, mesh.go): вернувшаяся/пересозданная
реплика (новый node ID над пустым томом) добавляется в layout — Garage дореплицирует
на неё; роль исчезнувшей ноды удаляется только при полном комплекте живых реплик,
чтобы транзиент не вызывал лишних ребалансов.

**skip-dead-nodes.** Удаление ноды заставляет Garage сливать (drain) её данные перед
финализацией, но переехавшая реплика получает новый node ID, а старый остаётся
**мёртвым** и слить с него нечего — кластер навсегда виснет в `degraded`, блокируя и
готовность ObjectStore, и следующий шаг ребаланса (health-gate). Admin API v1 такого
не умеет, поэтому контроллер, увидев `degraded` с недоступными storage-нодами,
принудительно двигает layout командой `garage layout skip-dead-nodes` (exec в под,
best-effort, повторяется каждый reconcile до расчистки).
