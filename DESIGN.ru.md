...
текущему числу узлов. Пересчёт запрещён и **fail-closed**: если ConfigMap
`garage.toml` уже существует, но фактор в нём отсутствует/не парсится, reconcile
возвращает ошибку (а не подставляет вычисленное значение) — иначе возможна
попытка смены `replication_factor` на живом кластере, которую Garage запрещает.
Хеш конфига проставляется в pod-template (аннотация
...
- `BucketClaim`: greenfield (`spec.objectStoreRef`) и brownfield
  (`spec.existingBucketName`) **взаимоисключаются** — ровно одно из двух (CEL).
  Раньше при обоих заданных полях заявка молча трактовалась как brownfield.
- Зарезервированный префикс `claim-` может использовать только контроллер:
  webhook разрешает его лишь запросам от **service account'а модуля**
  (по `AdmissionReview.userInfo`), а не по user-settable лейблу `bucket-origin` —
  иначе администратор мог бы поставить лейбл и занять greenfield-пространство имён.
...
`BucketClaimPolicyReconciler` watch'ит и `Bucket`: политика, созданная раньше
своего `bucketRef`-бакета, покидает `Pending` сразу как бакет появится (а не по
таймеру resync).
...
2. Через admin/S3 API бэкенда создать бакет (если нет), применить policy/quota.
   Провижининг backend-ресурсов **идемпотентен**: для Ceph RGW `CephObjectStore`
   и `CephObjectStoreUser` перезаписываются (и бампают generation → пересинк
   Rook) только при фактическом изменении управляемых полей, а не на каждом
   reconcile.
3. Финализатор: при удалении — удалить бакет только при `reclaimPolicy: Delete`.
   Под `Retain` данные сохраняются полностью; для Ceph RGW это означает, что
   сохраняется **и бакет, и его владелец-пользователь** (`CephObjectStoreUser`) —
   удаление владельца осиротило бы сохранённый бакет. Владелец удаляется только
   вместе с бакетом (при `Delete`). Финализатор снимается только когда
   `ObjectStore` действительно исчез (`NotFound`) или бэкенд неразрешим
   потому-что-удалён; при транзиентной ошибке reconcile повторяется, а финализатор
   **не снимается** — иначе backend-ресурсы осиротели бы.
...
2. Выдать отдельную пару ключей (Garage key / SeaweedFS identity / Ceph RGW user
   ...
   ключа атомарна по эффекту: если у Garage создание ключа прошло, а grant на
   бакет упал, свежесозданный ключ **откатывается** (удаляется), иначе он утёк бы
   орфаном, а следующий reconcile сминтил бы ещё один.
3. Записать `Secret` в namespace доступа (§4.4), проставить `status.secretRef`.
4. Финализатор: при удалении `BucketAccess` — отозвать ключ. Снимается только
   когда заявка действительно исчезла или бэкенд неразрешим потому-что-удалён;
