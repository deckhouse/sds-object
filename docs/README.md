...
| `BucketClaim` (`bc`) | Namespaced | The tenant request for a bucket: greenfield (provisions its own private bucket) or brownfield (binds a Shared bucket, gated by `BucketPolicy`). |
...
- The controller reconciles each `BucketClaim`: a greenfield claim provisions its own private `Bucket`; a brownfield claim binds an existing Shared `Bucket` once a `BucketPolicy` allows its namespace.
