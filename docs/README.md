...
|-------------|---------|------------------|----------|
| `System` | Garage | StatefulSet on control-plane nodes (3 replicas, or a single one with `systemBucket.singleReplica`), node-sticky local PV | Platform/system needs (backups, registry, logs). |
